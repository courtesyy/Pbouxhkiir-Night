...
CardChoice
	emits signal card_selected(card, player)
	card can be: “orbit”, “ship”, “worm”
	player can be: “xeno”, “annie”, “robot” 
DialogueScene (not done) 
	emits signal dialogue_done()
	(emits when clicked)
...
checkCard (not done)
randomCardType() (not done)
changeSprite(char, sprite) (change a character sprite ($Xeno, $Annie, $Robot) in “Characters.tscn” to a different sprite) (not done)
changeAggression(character) (increase character aggression by 1) (not done)

Variables
var card #used per-turn to store the card used, e.g. if we generate a random card that turn (not done)
ending #values: annie, robot, xeno, neutral. Determines which splash screen shows at the end. (not done)
choiceMode #values are either “card” or “dialogue”. Set this before a choice comes up to display the correct UI.
...
==turn5annie==
ANNIE: Hey human, do you have Gooeddoby? 
...
*[next]->turn6player
==turn6xenoanniefalse==
(doesn’t) ANNIE: Nope! Fish your oversized heart out.
*[next]->turn6player
...
*[nobody was targeted more than 3 times] -> endingneutral
*[robot was targeted 4 or more times] -> endingrobot
*[annie was targeted 4 or more times] -> endingannie
*[xeno was targeted 7 times] -> endingxeno
